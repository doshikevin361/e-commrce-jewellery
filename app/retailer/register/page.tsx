...
export default function RetailerRegistrationPage() {
  ...
  /** Shown under email when /api/verify/check-retailer-email reports already registered */
  const [retailerEmailCheckError, setRetailerEmailCheckError] = useState<string | null>(null);
  ...
  const handleSendOTP = async () => {
    setRetailerEmailCheckError(null);
    ...
    // Check if email is already registered as retailer (early validation)
    setLoading(true);
    try {
      const checkRes = await fetch(
        `/api/verify/check-retailer-email?email=${encodeURIComponent(formData.email)}`
      );
      ...
      if (!checkRes.ok) {
        ...
        setRetailerEmailCheckError(msg);
        toast({
          title: 'Error',
          description: msg,
          variant: 'destructive',
      ...
      if (exists) {
        const msg =
          'A retailer with this email is already registered. Use a different email or sign in.';
        setRetailerEmailCheckError(msg);
        toast({
      ...
      console.error('Retailer email check error:', e);
  ...
  const handleSupplierDetailsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          data.message ||
          'Registration submitted! Your retailer account is pending admin approval.',
        variant: 'success',
      });
      ...
      <header className='bg-white shadow-sm border-b'>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4'>
          <div className='flex items-center justify-between'>
            ...
            <Link href='/retailer/login' className='text-web font-medium hover:underline'>
        ...
        {!showRegistrationSteps ? (
          <div className='bg-white rounded-lg shadow-md p-8'>
            <div className='text-center mb-8'>
              <h1 className='text-3xl font-bold text-web mb-2'>Retailer registration</h1>
