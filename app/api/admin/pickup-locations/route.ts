...
import { PICKUP_LOCATIONS_COLLECTION, mongoFilterPickupListAdminVendor } from '@/lib/pickup-locations';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filter = mongoFilterPickupListAdminVendor(user!);
    const list = await db
    ...
    console.error('[pickup-locations] GET', e);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const ownerType = user!.role === 'vendor' ? 'vendor' : 'admin';
    const ownerId = user!.role === 'vendor' ? user!.id : 'platform';
    ...
    console.error('[pickup-locations] POST', e);
