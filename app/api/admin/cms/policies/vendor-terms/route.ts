...
const POLICY_TYPE = 'vendor_terms';
const DEFAULT_TITLE = 'Vendor Terms & Conditions';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[vendor-terms] Failed to fetch policy:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[vendor-terms] Failed to save policy:', error);
    return NextResponse.json({ error: 'Failed to save vendor terms' }, { status: 500 });
  }
}
