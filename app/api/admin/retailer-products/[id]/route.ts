...
/**
 * GET - Fetch single retailer product for admin edit page.
 * Same response shape as retailer my-products [id] for form compatibility.
 */
export async function GET(
  ...
  try {
    ...
    const product = await db.collection('retailer_products').findOne({ _id: new ObjectId(id) });
    if (!product) {
    ...
    const retailerIdObj = p.retailerId instanceof ObjectId ? p.retailerId : p.retailerId ? new ObjectId(String(p.retailerId)) : null;
    ...
    try {
      ...
      const relatedQuery: Record<string, unknown> = { _id: { $ne: new ObjectId(id) }, $or: statusOr };
      if (retailerIdObj) relatedQuery.retailerId = retailerIdObj;
      if (storedRelatedIds.length > 0) {
        const ids = storedRelatedIds.map((x) => new ObjectId(x));
        const resolved = await db
          .collection('retailer_products')
          .find({ _id: { $in: ids }, ...(retailerIdObj ? { retailerId: retailerIdObj } : {}), $or: statusOr })
      ...
      } else if (currentCategory && retailerIdObj) {
    ...
    console.error('[Admin Retailer Products] GET error:', e);
...
/**
 * PATCH - Update retailer product (admin). Same payload as retailer PATCH.
 */
export async function PATCH(
  ...
  try {
    ...
    console.error('[Admin Retailer Products] PATCH error:', e);
    return NextResponse.json({ error: 'Failed to update product' }, { status: 500 });
  }
}

/**
 * DELETE - Remove retailer product (admin only).
 */
export async function DELETE(
  ...
  try {
    ...
    console.error('[Admin Retailer Products] DELETE error:', e);
