...
export function RetailerSubscriptionPanel() {
  ...
  const fetchPlans = useCallback(async () => {
    try {
      const res = await fetch('/api/subscription/plans?role=retailer', { cache: 'no-store' });
      if (!res.ok) return;
      const data = await res.json();
      setPlans(data.plans || []);
    } catch (e) {
  ...
  if (!status?.subscriptionEnabled) {
    return (
      <div className="max-w-2xl">
        <h1 className="text-2xl font-semibold mb-2">Retailer Subscription</h1>
        <p className="text-muted-foreground">
          Subscription is not required at this time. You have full access to retailer features.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <h1 className="text-2xl font-semibold">Retailer Subscription</h1>
